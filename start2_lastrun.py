...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/alisaloosen/Documents/GitHub/RewardEffortTask/start2_lastrun.py',
...
image_shipCrash = visual.ImageStim(
    win=win,
    name='image_shipCrash', 
    image='crashedShip.png', mask=None, anchor='center',
    ori=0.0, pos=[0.7, -0.5], size=[0.6, 0.7],
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=-3.0)
image_shipFull = visual.ImageStim(
    win=win,
    name='image_shipFull', 
    image='fullShip.png', mask=None, anchor='center',
    ori=0.0, pos=[0.7, 0.5], size=[0.6, 0.7],
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=-4.0)
...
for thisMaintask in maintask:
    ...
    # keep track of which components have finished
    Feedback_2Components = [feedbackMsg, image_rubies, image_shipCrash, image_shipFull]
    ...
    while continueRoutine:
        ...
        # *image_shipCrash* updates
        
        # if image_shipCrash is starting this frame...
        if image_shipCrash.status == NOT_STARTED and crashShow==3:
            # keep track of start time/frame for later
            image_shipCrash.frameNStart = frameN  # exact frame index
            image_shipCrash.tStart = t  # local t and not account for scr refresh
            image_shipCrash.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(image_shipCrash, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'image_shipCrash.started')
            # update status
            image_shipCrash.status = STARTED
            image_shipCrash.setAutoDraw(True)
        
        # if image_shipCrash is active this frame...
        if image_shipCrash.status == STARTED:
            # update params
            pass
        
        # if image_shipCrash is stopping this frame...
        if image_shipCrash.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > image_shipCrash.tStartRefresh + feedbackTime-frameTolerance:
                # keep track of stop time/frame for later
                image_shipCrash.tStop = t  # not accounting for scr refresh
                image_shipCrash.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'image_shipCrash.stopped')
                # update status
                image_shipCrash.status = FINISHED
                image_shipCrash.setAutoDraw(False)
        
        # *image_shipFull* updates
        
        # if image_shipFull is starting this frame...
        if image_shipFull.status == NOT_STARTED and crashShow==3:
            # keep track of start time/frame for later
            image_shipFull.frameNStart = frameN  # exact frame index
            image_shipFull.tStart = t  # local t and not account for scr refresh
            image_shipFull.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(image_shipFull, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'image_shipFull.started')
            # update status
            image_shipFull.status = STARTED
            image_shipFull.setAutoDraw(True)
        
        # if image_shipFull is active this frame...
        if image_shipFull.status == STARTED:
            # update params
            pass
        
        # if image_shipFull is stopping this frame...
        if image_shipFull.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > image_shipFull.tStartRefresh + feedbackTime-frameTolerance:
                # keep track of stop time/frame for later
                image_shipFull.tStop = t  # not accounting for scr refresh
                image_shipFull.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'image_shipFull.stopped')
                # update status
                image_shipFull.status = FINISHED
                image_shipFull.setAutoDraw(False)
