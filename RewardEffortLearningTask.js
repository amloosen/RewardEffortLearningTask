...
psychoJS.openWindow({
  fullscr: false,
  color: new util.Color([(- 1.0), (- 1.0), (- 0.2471)]),
  units: 'norm',
  waitBlanking: true
});
...
const maintaskLoopScheduler = new Scheduler(psychoJS);
flowScheduler.add(maintaskLoopBegin(maintaskLoopScheduler));
flowScheduler.add(maintaskLoopScheduler);
flowScheduler.add(maintaskLoopEnd);
...
psychoJS.start({
  ...
  resources: [
    // resources:
    {'name': 'pract1.xlsx', 'path': 'pract1.xlsx'},
    {'name': 'trialTypes.xlsx', 'path': 'trialTypes.xlsx'},
    {'name': 'image3.png', 'path': 'image3.png'},
    {'name': 'image4.png', 'path': 'image4.png'},
    {'name': 'image6.png', 'path': 'image6.png'},
    {'name': 'image5.png', 'path': 'image5.png'},
    {'name': 'image2.png', 'path': 'image2.png'},
    {'name': 'image7.png', 'path': 'image7.png'},
    {'name': 'image1.png', 'path': 'image1.png'},
    {'name': 'example_tank.mov', 'path': 'example_tank.mov'},
    {'name': 'TankOnlyEmpt-01.png', 'path': 'TankOnlyEmpt-01.png'},
    {'name': 'image1.png', 'path': 'image1.png'},
    {'name': 'image7.png', 'path': 'image7.png'},
    {'name': 'fuelShipWhite.png', 'path': 'fuelShipWhite.png'},
    {'name': 'default.png', 'path': 'https://pavlovia.org/assets/default/default.png'},
    {'name': 'fuelShipFull-01.png', 'path': 'fuelShipFull-01.png'},
  ]
});
...
async function experimentInit() {
  ...
  example_tank_recClock = new util.Clock();
  example_tank_rec = new visual.MovieStim({
    win: psychoJS.window,
    name: 'example_tank_rec',
    units: psychoJS.window.units,
    movie: 'example_tank.mov',
    pos: [0, 0.15],
    anchor: 'center',
    size: [0.1, 0.5],
    ori: 0.0,
  ...
  // Run 'Begin Experiment' code from code_JS
  practice_intro_text1.setAlignHoriz("center");
  practice_intro_text2.setAlignHoriz("center");
  ...
  tank_2 = new visual.ImageStim({
    ...
    image : 'TankOnlyEmpt-01.png', mask : undefined,
    anchor : 'center',
  ...
  trial_select = new core.Keyboard({psychoJS: psychoJS, clock: new util.Clock(), waitForStart: true});
  ...
  coins_little_2 = new visual.ImageStim({
    ...
    image : 'image1.png', mask : undefined,
    anchor : 'center',
  ...
  coins_many_2 = new visual.ImageStim({
    ...
    image : 'image7.png', mask : undefined,
    anchor : 'center',
  ...
  fuel_full = new visual.ImageStim({
    ...
    image : 'fuelShipFull-01.png', mask : undefined,
    anchor : 'center',
...
function practice_instructionsRoutineBegin(snapshot) {
  return async function () {
    ...
    practice_instructionsComponents.push(example_tank_rec);
...
function practice_instructionsRoutineEachFrame() {
  return async function () {
    ...
    if (t >= 0.1 && example_tank_rec.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      example_tank_rec.tStart = t;  // (not accounting for frame time here)
...
function maintaskLoopBegin(maintaskLoopScheduler, snapshot) {
  return async function() {
    TrialHandler.fromSnapshot(snapshot); // update internal variables (.thisN etc) of the loop
    
    // set up handler to look after randomisation of conditions etc
    maintask = new TrialHandler({
      psychoJS: psychoJS,
      nReps: 1, method: TrialHandler.Method.SEQUENTIAL,
      extraInfo: expInfo, originPath: undefined,
      trialList: 'trialTypes.xlsx',
      seed: undefined, name: 'maintask'
    });
    psychoJS.experiment.addLoop(maintask); // add the loop to the experiment
    currentLoop = maintask;  // we're now the current loop
    
    // Schedule all the trials in the trialList:
    for (const thisMaintask of maintask) {
      snapshot = maintask.getSnapshot();
      maintaskLoopScheduler.add(importConditions(snapshot));
      maintaskLoopScheduler.add(reward_ratingRoutineBegin(snapshot));
      maintaskLoopScheduler.add(reward_ratingRoutineEachFrame());
      maintaskLoopScheduler.add(reward_ratingRoutineEnd(snapshot));
      maintaskLoopScheduler.add(crossRoutineBegin(snapshot));
      maintaskLoopScheduler.add(crossRoutineEachFrame());
      maintaskLoopScheduler.add(crossRoutineEnd(snapshot));
      maintaskLoopScheduler.add(effort_ratingRoutineBegin(snapshot));
      maintaskLoopScheduler.add(effort_ratingRoutineEachFrame());
      maintaskLoopScheduler.add(effort_ratingRoutineEnd(snapshot));
      maintaskLoopScheduler.add(crossRoutineBegin(snapshot));
      maintaskLoopScheduler.add(crossRoutineEachFrame());
      maintaskLoopScheduler.add(crossRoutineEnd(snapshot));
      maintaskLoopScheduler.add(main_trialRoutineBegin(snapshot));
      maintaskLoopScheduler.add(main_trialRoutineEachFrame());
      maintaskLoopScheduler.add(main_trialRoutineEnd(snapshot));
      maintaskLoopScheduler.add(Feedback_2RoutineBegin(snapshot));
      maintaskLoopScheduler.add(Feedback_2RoutineEachFrame());
      maintaskLoopScheduler.add(Feedback_2RoutineEnd(snapshot));
      maintaskLoopScheduler.add(maintaskLoopEndIteration(maintaskLoopScheduler, snapshot));
    }
...
async function maintaskLoopEnd() {
  // terminate loop
  psychoJS.experiment.removeLoop(maintask);
  // update the current loop from the ExperimentHandler
  if (psychoJS.experiment._unfinishedLoops.length>0)
    currentLoop = psychoJS.experiment._unfinishedLoops.at(-1);
  else
...
function maintaskLoopEndIteration(scheduler, snapshot) {
  // ------Prepare for next entry------
  return async function () {
    if (typeof snapshot !== 'undefined') {
      // ------Check if user ended loop early------
...
function practiceRoutineBegin(snapshot) {
  return async function () {
    ...
    achieved = 0;
    firstPump = false;
    loop_start_time = new core.Clock();
    
    // Run 'Begin Routine' code from countdownEasy_3
    if ((! countdownStarted)) {
        countdownClock = new core.CountdownTimer(timeAvailable);
...
function practiceRoutineEachFrame() {
  return async function () {
    ...
    if (((timeRemaining <= 0.0) || (((- 0.5) + batterySize) >= effTresh))) {
        continueRoutine = false;
        countdownStarted = false;
    }
    ...
    if (barBody_2.status === PsychoJS.Status.STARTED){ // only update if being drawn
      ...
      barBody_2.setSize([0.15, batterySize], false);
    }
...
function practiceRoutineEnd(snapshot) {
  return async function () {
    ...
    max_press_pract.push((npumps / t));
    max_press_pSec_pract = Math.max(max_press_pract);
...
function InstructionsRoutineBegin(snapshot) {
  return async function () {
    ...
    if ((! countdownStarted)) {
        countdownClock = new core.CountdownTimer(5);
        countdownStarted = true;
    }
    
    trial_select.keys = undefined;
    trial_select.rt = undefined;
    _trial_select_allKeys = [];
    // keep track of which components have finished
    InstructionsComponents = [];
    InstructionsComponents.push(instrText);
    InstructionsComponents.push(trial_select);
...
function InstructionsRoutineEachFrame() {
  return async function () {
    ...
    if (t >= 0 && trial_select.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      trial_select.tStart = t;  // (not accounting for frame time here)
      trial_select.frameNStart = frameN;  // exact frame index
      
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function() { trial_select.clock.reset(); });  // t=0 on next screen flip
      psychoJS.window.callOnFlip(function() { trial_select.start(); }); // start on screen flip
      psychoJS.window.callOnFlip(function() { trial_select.clearEvents(); });
    }

    if (trial_select.status === PsychoJS.Status.STARTED) {
      let theseKeys = trial_select.getKeys({keyList: ['return'], waitRelease: false});
      _trial_select_allKeys = _trial_select_allKeys.concat(theseKeys);
      if (_trial_select_allKeys.length > 0) {
        trial_select.keys = _trial_select_allKeys.map((key) => key.name);  // storing all keys
        trial_select.rt = _trial_select_allKeys.map((key) => key.rt);
        trial_select.duration = _trial_select_allKeys.map((key) => key.duration);
        // a response ends the routine
        continueRoutine = false;
      }
    }
...
function InstructionsRoutineEnd(snapshot) {
  return async function () {
    ...
    // update the trial handler
    if (currentLoop instanceof MultiStairHandler) {
      currentLoop.addResponse(trial_select.corr, level);
    }
    psychoJS.experiment.addData('trial_select.keys', trial_select.keys);
    if (typeof trial_select.keys !== 'undefined') {  // we had a response
        psychoJS.experiment.addData('trial_select.rt', trial_select.rt);
        psychoJS.experiment.addData('trial_select.duration', trial_select.duration);
        routineTimer.reset();
        }
...
function reward_ratingRoutineEnd(snapshot) {
  return async function () {
    ...
    console.log("Scale Value:", rewscale_value);
...
function effort_ratingRoutineEnd(snapshot) {
  return async function () {
    ...
    console.log("Scale Value:", effscale_value);
  ...
  return async function () {
    ...
    if ((maintask.thisN === 0)) {
        if ((max_press_pSec_pract === 0)) {
            ...
            max_press_pSec = util.round(max_press_pSec_pract);
        }
        npumps_pSec = [0];
    }
    ...
    console.log(batterySizeIncr);
    
    // keep track of which components have finished
    main_trialComponents = [];
    main_trialComponents.push(text_6);
...
function main_trialRoutineEachFrame() {
  return async function () {
    ...
    if (barBody.status === PsychoJS.Status.STARTED){ // only update if being drawn
      ...
      barBody.setSize([0.15, batterySize], false);
    }
...
function main_trialRoutineEnd(snapshot) {
  return async function () {
    ...
    tscore += rscore;
    npumps_pSec.push((npumps / t));
    console.log(max_press_pSec);
    console.log(npumps_pSec.slice((- 1))[0]);
    if ((max_press_pSec < npumps_pSec.slice((- 1))[0])) {
        max_press_pSec = npumps_pSec.slice((- 1))[0];
        console.log("new max:", max_press_pSec);
    }
    maintask.addData("threshold", effTresh);
...
function Feedback_2RoutineBegin(snapshot) {
  return async function () {
    ...
    Feedback_2Components.push(feedbackMsg);
    Feedback_2Components.push(image_rubies);
    Feedback_2Components.push(photodiode_ft1);
